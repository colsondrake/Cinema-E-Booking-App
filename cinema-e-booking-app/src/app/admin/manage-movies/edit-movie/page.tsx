...
export default function EditMoviePage() {
  ...
  // Form state (same shape as AddMoviePage)
  ...
  useEffect(() => {
    if (!movieId) {
      setError("Missing movieId in URL");
      return;
    }
    ...
    const fetchMovie = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Failed to load movie (status ${res.status})`);
        }
        ...
        setStatus(
          STATUSES.includes(data.status) ? data.status : STATUSES[0]
        );
      } catch (err: any) {
  ...
  // Genre helpers (same as AddMoviePage)
  const addGenre = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        posterUrl,
        trailerUrl,
        showtimes: [], // you can load/preserve these separately if needed
        status,
      };
    ...
    <section className="flex items-center justify-center min-h-screen bg-[#0b1727] text-white py-10 md:py-12">
      <div className="w-full max-w-3xl bg-[#17233a] border border-[#1f2d49] rounded-2xl p-8 shadow-lg">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold">Edit Movie</h1>
          <p className="text-gray-300 mt-2">
            Update this movie&apos;s details and save your changes.
          </p>
        </div>
        ...
        {loading ? (
          <p className="text-gray-300 text-sm">Loading movie…</p>
        ) : (
